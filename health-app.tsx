  ...
  async function fetchAdvice(){
    ...
    try{
      ...
      const summary=recent.map(r=>{
        const f=r.fields;
        let s=`[${r.date} ${r.time}] ${DETAIL_CATS[r.cat].label}: `;
        if(r.cat==="weight") s+=`体重${f.weight}kg${f.fat?` 体脂肪${f.fat}%`:""}`;
        else if(r.cat==="meal") s+=`${f.mealType||""}${f.calories?` ${f.calories}kcal`:""}`;
        else if(r.cat==="exercise") s+=`${f.exType} ${f.duration||""}分`;
        else if(r.cat==="sleep") s+=`${f.bedtime||""}〜${f.wakeup||""} ${f.quality||""}`;
        else if(r.cat==="condition") s+=`${f.overall||""}${f.symptoms?` ${f.symptoms}`:""}`;
        else if(r.cat==="period") s+=`${f.periodType||""} ${f.flow||""}`;
        else if(r.cat==="bowel") s+=`${f.bowelResult||""}${f.bowelType?` (${f.bowelType})`:""}`
        if(r.note) s+=` (${r.note})`;
        return s;
      }).join("\n");
      const prompt=mode==="question"&&question.trim()?`以下は私の健康記録です。\n\n${summary}\n\n質問：${question}`:`以下は私の健康記録です。分析してアドバイスをください。\n\n${summary}`;
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:"あなたは丁寧で親切な健康アドバイザーです。ユーザーの健康記録を分析し、具体的で実践しやすいアドバイスを日本語でしてください。医療的診断は行わず一般的な健康管理のアドバイスのみ提供し、必要に応じて専門家への相談を促してください。箇条書きや絵文字を使って読みやすくしてください。",messages:[{role:"user",content:prompt}]})});
      const data=await res.json();
      setAdvice(data.content?.map(b=>b.text||"").join("")||"取得できませんでした。");
    }catch(e){setAdvice("エラーが発生しました。");}
    setLoading(false);
  }
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <Card style={{background:C.mintLight,borderColor:C.mint}}>
        <p style={{margin:"0 0 4px",fontSize:13,fontWeight:700,color:C.mint}}>✦ AIアドバイス</p>
        <p style={{margin:0,fontSize:12,color:C.sub,lineHeight:1.7}}>記録データをもとにClaudeが分析します。医療的診断ではありませんので、気になる症状は医療機関へ。</p>
      </Card>
      <div style={{display:"flex",gap:8}}>
        {[{id:"auto",label:"📊 記録を分析"},{id:"question",label:"💬 質問する"}].map(m=>(
          <button key={m.id} onClick={()=>setMode(m.id)} style={{flex:1,padding:"10px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:13,background:mode===m.id?C.mint:C.bg,color:mode===m.id?"#fff":C.sub,border:`1.5px solid ${mode===m.id?C.mint:C.border}`}}>{m.label}</button>
        ))}
      </div>
      {mode==="question"&&<textarea value={question} onChange={e=>setQuestion(e.target.value)} placeholder="例：最近疲れやすいのですが…" rows={3} style={{padding:"12px",borderRadius:10,border:`1.5px solid ${C.border}`,background:C.bg,fontSize:14,color:C.ink,resize:"vertical",outline:"none",lineHeight:1.6}}/>}
      <button onClick={fetchAdvice} disabled={loading||records.length===0} style={{width:"100%",padding:"13px",borderRadius:12,border:"none",background:records.length===0||loading?C.border:C.mint,color:"#fff",fontSize:15,fontWeight:700,cursor:records.length===0||loading?"not-allowed":"pointer"}}>{loading?"分析中…":"✦ AIにアドバイスをもらう"}</button>
      {records.length===0&&<p style={{fontSize:12,color:C.sub,textAlign:"center"}}>先に記録を入力してください</p>}
      {loading&&<Card style={{textAlign:"center",padding:36}}><p style={{color:C.sub}}>🔍 分析しています…</p></Card>}
      {advice&&!loading&&<Card><p style={{margin:"0 0 10px",fontSize:12,fontWeight:700,color:C.mint}}>✦ AIからのアドバイス</p><div style={{fontSize:14,color:C.ink,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{advice}</div></Card>}
    </div>
  );
}
